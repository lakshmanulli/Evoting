...
def WebCam(request):
    if request.method == 'GET':
        ...
        with open('D:/EVoting/EVotingApp/static/photo/test.png', 'wb') as f:
            f.write(data)
        f.close()
        context= {'data':"done"}
...
def ValidateUser(request):
    if request.method == 'POST':
        ...
        img = cv2.imread('D:/EVoting/EVotingApp/static/photo/test.png')
        faces = face_detection.detectMultiScale(img,scaleFactor=1.1,minNeighbors=5,minSize=(30,30),flags=cv2.CASCADE_SCALE_IMAGE)
        print("==================="+str(len(faces)))   
        if len(faces) > 0:
            if load_model == 0:
                with open('D:/EVoting/model/model.json', "r") as json_file:
                    loaded_model_json = json_file.read()
                    classifier = model_from_json(loaded_model_json)
                classifier.load_weights("D:/EVoting/model/model_weights.h5")
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      filename = fs.save('D:/EVoting/EVotingApp/static/profiles/'+cname+'.png', myfile)
...
def Signup(request):
    if request.method == 'POST':
      ...
      fs = FileSystemStorage()
      filename = fs.save('D:/EVoting/EVotingApp/static/profiles/'+username+'.png', myfile)
